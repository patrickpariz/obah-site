  ...
  return (
    <>
      <nav style={{
        ...
        {/* Desktop links */}
        <ul style={{ display: 'flex', alignItems: 'center', gap: '32px', listStyle: 'none', margin: 0, padding: 0 }} className="nav-desktop">
          {links.map(item => (
            <li key={item.label}>
          ...
          <li>
            <a href="https://ingressos.obahoficial.com.br" target="_blank" style={{
              background: '#F5A623', color: '#fff', padding: '10px 22px',
      ...
      {menuOpen && (
        <div style={{
          ...
          background: 'rgba(251,247,239,0.98)', backdropFilter: 'blur(12px)',
          borderBottom: '1px solid rgba(245,166,35,0.15)',
          ...
          {links.map(item => (
            <a key={item.label} href={item.href}
              ...
              style={{
                fontSize: '18px', fontWeight: 400, letterSpacing: '0.06em',
                textTransform: 'uppercase', textDecoration: 'none',
                color: activeSection === item.id ? '#F5A623' : '#3D3D3D',
              }}>
              {item.label}
          ...
          <a href="https://ingressos.obahoficial.com.br" target="_blank"
            onClick={() => setMenuOpen(false)}
            style={{
              background: '#F5A623', color: '#fff', padding: '14px 22px',
              borderRadius: '2px', fontSize: '14px', fontWeight: 500,
